...
pub(crate) fn text_visible_width(value: &str) -> usize {
    strip_ansi(value).chars().map(rune_width).sum()
}

pub(crate) fn text_visible_chars(value: &str) -> Vec<String> {
    strip_ansi(value).chars().map(|c| c.to_string()).collect()
}

pub(crate) fn text_truncate_to_width(value: &str, width: usize) -> String {
    let mut out = String::new();
    let mut used = 0usize;
...
pub(crate) fn text_pad_to_width(value: &str, width: usize) -> String {
