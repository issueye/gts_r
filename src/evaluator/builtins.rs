...
pub fn register_globals(vm: &Rc<VirtualMachine>) {
    ...
    let println_fn: FnPtr = Rc::new(|_ctx, args| {
    ...
    let print_fn: FnPtr = Rc::new(|_ctx, args| {
        for a in args {
            print!("{}", a.inspect());
        }
        Object::Undefined
...
fn register_timers(vm: &Rc<VirtualMachine>) {
    ...
    let sleep_async: FnPtr = Rc::new(|_ctx, args| {
        let ms = as_num(args.get(0)) as u64;
        let promise = Promise::new();
        if ms > 0 {
            std::thread::sleep(std::time::Duration::from_millis(ms));
        }
            ...
            Some(Object::Array(arr)) => {
                ...
                for (i, item) in items.into_iter().enumerate() {
                    match item {
                            ...
                            if p.state() == PromiseState::Rejected {
                                ...
                                if remaining.fetch_sub(1, std::sync::atomic::Ordering::SeqCst) == 1
                                {
                                    let collected: Vec<Object> = results
                                        ...
                                        .collect();
                                    promise.resolve(Object::Array(Rc::new(RefCell::new(
                                        ArrayData {
                                            elements: collected,
                                        },
                                    ))));
                        ...
                        other => {
                            results.borrow_mut()[i] = Some(other);
                            if remaining.fetch_sub(1, std::sync::atomic::Ordering::SeqCst) == 1 {
                                let collected: Vec<Object> = results
                                    .borrow_mut()
                                    .iter()
    ...
    let date_constructor: FnPtr = Rc::new(|_ctx, args| {
        ...
        if args.len() >= 2 {
            ...
            let month = match args.get(1) {
                Some(Object::Number(n)) => (*n as u32 + 1).max(1).min(12), // JS months are 0-indexed
                _ => 1,
            };
            let day = match args.get(2) {
                Some(Object::Number(n)) => (*n as u32).max(1).min(31),
                _ => 1,
            };
...
fn arr_unshift(ctx: &mut CallContext, args: &[Object]) -> Object {
    if let Some(a) = active_array(ctx) {
        ...
        new_elems.extend(arr.elements.drain(..));
...
fn arr_copy_within(ctx: &mut CallContext, args: &[Object]) -> Object {
    if let Some(a) = active_array(ctx) {
        ...
        let target = normalize_index(as_num(args.get(0)) as isize, length) as usize;
        ...
        let copy_count = if end > start { end - start } else { 0 };
...
fn str_repeat(ctx: &mut CallContext, args: &[Object]) -> Object {
    if let Some(s) = active_string(ctx) {
        let n = as_num(args.get(0)) as usize;
...
fn str_pad_start(ctx: &mut CallContext, args: &[Object]) -> Object {
    if let Some(s) = active_string(ctx) {
        let target = as_num(args.get(0)) as usize;
    ...
    match &args[0] {
        Object::Regexp(re) => {
            ...
            if re.flags.contains('g') {
                ...
                return Object::Array(Rc::new(RefCell::new(ArrayData { elements: matches })));
            } else {
...
fn num_to_fixed(ctx: &mut CallContext, args: &[Object]) -> Object {
    if let Some(n) = active_number(ctx) {
        let digits = as_num(args.get(0)) as usize;
...
fn num_to_exponential(ctx: &mut CallContext, args: &[Object]) -> Object {
    if let Some(n) = active_number(ctx) {
        let digits = if args.is_empty() {
            ...
            let precision = as_num(args.get(0)) as usize;
