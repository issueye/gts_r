...
- [x] 6.4 阶段 6 契约门（VM 单跑全绿）：
  - [x] `09_errors`（仓库当前无独立 `09_errors` 聚合目录，改以现有 parity 目录中的错误处理明细 fixture 验收）
  - [x] `try_catch` `try_finally_no_throw` `catch_finally_order`
  - [x] `throw_catch_string` `throw_catch_error`
  - [ ] `match_no_arm_catch`（依赖阶段 7 Match 编译；当前错误处理 catch/unwind 已具备，Match AST 未覆盖）
...
- [x] 7.2 5 种 Pattern：`Literal(Eq)/Ident(绑定)/Wildcard(无条件)/Or(任一)/Range(Ge+Le)`
  - 证据：7.1 已实现 `compile_pattern_test` 覆盖 `Pattern::Literal`→`Eq`、`Pattern::Ident`/`Wildcard`→无条件命中并绑定、`Pattern::Or`→逐 alternative 测试任一命中、`Pattern::Range`→`Ge` + `Le/Lt` 范围测试；新增 fixture `match_or`、`match_range`、`match_ident_binding`，树遍历 oracle 分别为 `match-or=primary`、`match-range=medium`、`match-ident-binding=id:gts`，并纳入 `tests/bytecode_parity.rs`；`cargo fmt --all --check` passed；`cargo test --lib bytecode` 95 passed；`cargo test --test bytecode_parity -- --nocapture` 1 passed
- [x] 7.3 `MatchBody::{Expr,Block}` + `guard`
...
- [x] 7.4 类型注解：`type_check=true` 时插入运行期检查调用；默认 false 对齐树遍历
...
- [x] 8.3 循环依赖检测（复用现有 module cache）
  - 证据：`src/runtime/mod.rs` 新增 `eval_program_for_session`，在 `EXEC_MODE_BYTECODE` 下让入口脚本、`run_file_for_exports` 与 importer 加载的源模块都走 `bytecode::compile` + `bytecode::interpret`，默认树遍历路径不变；importer 继续复用现有 `module_cache` 的预插入占位 exports 对象，从而保持循环依赖 partial exports 语义；`src/object/mod.rs` 重导出 `EXEC_MODE_BYTECODE/EXEC_MODE_TREEWALK` 供运行时测试设置执行模式；新增 `tests/bytecode_modules.rs::bytecode_runtime_reuses_module_cache_for_circular_require` 覆盖 bytecode 模式下 `a -> b -> a` 循环 require 输出 `loading:done`；`cargo fmt --all --check` passed；`cargo test --test bytecode_modules -- --nocapture` 1 passed；`cargo test --lib bytecode` 107 passed；`cargo test --test bytecode_parity -- --nocapture` 1 passed
- [x] 8.4 阶段 8 契约门（VM 单跑全绿）：
  - [x] `relative_require` `nested_relative_require` `project_module_require` `directory_module_index` `module_cache` `module_exports_object` `import_default_like` `export_const` `export_function_alias`
...
- [x] 10.7 提交 `[bytecode-10] 全量交付与默认切换`
  - 证据：阶段 10 已拆分为单职责提交完成：`276b23c test(bytecode): run all parity fixtures`（10.1）、`11a16f2 test(bytecode): validate verification suites`（10.2）、`c2729d5 test(bytecode): audit compiler coverage`（10.3）、`e1e658d test(bytecode): add bench server performance gate`（10.4）、`cf2060e feat(bytecode): default sessions to bytecode`（10.5）、`72eabb9 docs(bytecode): keep treewalker fallback`（10.6）；终验覆盖 REQ-1/REQ-2/REQ-3、性能门、默认 Bytecode 切换与 legacy fallback 决策；阶段 10 收口提交记录当前项。
...
- [x] 11.2 默认执行后端下沉到 `VirtualMachine::new()`
  - 证据：`src/object/vm.rs` 将 `VirtualMachine::new()` 的 `exec_mode` 初始值从 `EXEC_MODE_TREEWALK` 改为 `EXEC_MODE_BYTECODE`，注释同步为 tree-walker 是 legacy、bytecode VM 是默认；`Session::new()` 仍显式写入 Bytecode，CLI 仍可通过 `--exec-mode=tree` 覆盖回树遍历；`tests/bytecode_default.rs::virtual_machine_new_defaults_to_bytecode_exec_mode` 直接断言底层 VM 默认 Bytecode，原 `session_new_defaults_to_bytecode_exec_mode` 继续覆盖 Session 默认；验证：`cargo fmt --all --check` passed、`cargo test --test bytecode_default -- --nocapture` 2 passed、`cargo test --test cli_flags cli_exec_mode_tree_keeps_treewalker_fallback -- --nocapture` 1 passed、`cargo test --test bytecode_parity -- --nocapture` 1 passed
...
**最后更新**：2026-06-23（阶段 11.2 已完成：默认执行后端已下沉到 `VirtualMachine::new()`；后续继续盘点 fallback-only 能力）
