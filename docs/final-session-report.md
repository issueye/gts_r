...
3. `Cargo.toml` - 新增 ureq 依赖

### 测试文件 (新增)
4. `tests/stdlib_p8_exec.rs` - exec 模块测试 (133 行, 6 测试)
5. `tests/stdlib_p8_http.rs` - http 模块测试 (107 行, 4 测试)
6. `tests/async_promise.rs` - Promise 测试 (131 行, 6 测试)
7. `tests/lang_match.rs` - match 表达式测试 (154 行, 6 测试)

### 文档
8. `docs/parity-matrix.md` - 更新 4 个模块状态
9. `docs/session-2-progress-report.md` - 详细进度报告
10. `docs/refactoring-progress-summary.md` - 第一阶段总结
...
### Missing (尚未实现)
❌ CLI: run-script, pack, dist, bundle, LSP, --check-types
...
### 高优先级 (核心功能)
1. **@std/net/http/server** - HTTP 服务器 (与 client 配套)
2. **Module resolver** - 包解析器 (影响模块系统)
3. **Language core 测试** - 与 Go 版本对照测试
4. **CLI: run-script** - 脚本执行支持

### 中优先级 (扩展功能)
5. **@std/db** - 数据库连接
6. **@std/net/socket** - Socket 支持
7. **@std/net/ws** - WebSocket 支持
8. **CLI: pack, dist** - 打包和分发

### 低优先级 (高级功能)
9. **LSP** - 语言服务器协议
10. **Type checker** - 类型检查器
11. **GTP plugins** - 插件系统
...
新增 **22 个测试**，全部通过。测试总数从 105 增加到 **127**。

虽然"完成剩余功能的重构"是一个宏大的目标，仍有大量工作待完成（HTTP 服务器、数据库、打包系统等），但本次会话已经成功实现了多个高优先级的核心模块，为后续工作奠定了坚实基础。
